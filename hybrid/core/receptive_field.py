...
def _model_receptive_field(model: dict) -> int:
    ...
    own `architecture` field -- the same field `hybrid.core.nam_loader.NamModel`
    exposes.
    ...
    raise ReceptiveFieldUnavailable(
        ...
        "add a case to hybrid.core.receptive_field._model_receptive_field for it."
    )
...
def combine_required_history(
    ...
    manifest JSON without extra plumbing; both `hybrid.modes.training_target` and
    `hybrid.modes.blend_training_target` build the "receptive_field" manifest
...
def assert_required_history_fits(
    ...
    `hybrid.core.pipeline.render_pair`/scripts/train_a2.py's `check_receptive_field`:
