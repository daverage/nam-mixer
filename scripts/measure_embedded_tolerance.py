...
from hybrid.training.embedded_completion import EMBEDDED_PACKAGE_MAX_ABS_ERROR, _sequential_warmup_samples
from hybrid.training.sequential_nam import package_embedded_artifacts

OUT = Path(sys.argv[1]) if len(sys.argv) > 1 else REPO / "work" / "embedded_tolerance"
OUT.mkdir(parents=True, exist_ok=True); SR = 48000
HEAD = REPO / "docs" / "history" / "Continuous Gain" / "phase4e" / "models" / "jcm800_P4E_B_s0.nam"
rng = np.random.default_rng(0)
t = np.arange(SR) / SR
irs = {
    "identity (1 tap)": np.array([1.0]),
    "short (2 taps)": np.array([1.0, 0.25]),
    "real V30 (24001 taps)": sf.read(REPO / "assets" / "nam_models" / "V30 LL 4FB 4x12 SM57 1.00in 0.0in SA73.wav", dtype="float32")[0],
...
dry0, _ = sf.read(REPO / "assets" / "di" / "moderate_brit.wav", dtype="float32"); dry0 = dry0[: 8 * SR]
inputs = {"DI 0 dB": dry0, "DI +12 dB": (dry0 * 10 ** (12 / 20)).astype(np.float32)}
head_model = load_nam(HEAD)
heads = {label: render(head_model, dry, SR, slim=SLIM_FULL) for label, dry in inputs.items()}  # once per input
seq_exe = find_sequential_nam_render_exe()
print(f"{'IR':34s} {'input':10s} {'scalar':>7s} {'peak|exp|':>9s} {'max|err|':>10s} {'p99.9|err|':>10s} {'max/peak':>9s}  {EMBEDDED_PACKAGE_MAX_ABS_ERROR:g}")
for name, ir in irs.items():
    ...
    for scalar in (1.0, 10 ** (-4 / 20)):
        ...
        art = package_embedded_artifacts(HEAD, d, cab, sample_rate=SR, final_scalar=scalar, base_name="measure")
        ...
        for label, dry in inputs.items():
            expected = apply_cab_ir(heads[label], prepared) * scalar
            ...
            flag = "FAIL" if err.max() > EMBEDDED_PACKAGE_MAX_ABS_ERROR else "ok"
            print(f"{name:34s} {label:10s} {scalar:7.3f} {peak:9.3f} {err.max():10.2e} {np.percentile(err, 99.9):10.2e} {err.max()/peak:9.1e}  {flag}")
        if name.startswith("real"):
            h = heads["DI 0 dB"]; actual = render(seq, dry0, SR, executable=seq_exe)
            faults = {
