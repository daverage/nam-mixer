...
int main(int argc, char* argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (arg == "--block-size")
    {
      ...
      if (end == argv[i + 1] || *end != '\0' || parsed < 1 || parsed > 65536)
      {
        std::cerr << "Error: --block-size must be an integer from 1 to 65536\n";
        return 1;
      }
      bufferSize = static_cast<int>(parsed);
      i++;
    }
    else
    {
  ...
  {
    std::cerr << "Usage: render [--slim <0.0-1.0>] [--block-size <frames>] <model.nam> <input.wav> [output.wav]\n";
    return 1;
  }
